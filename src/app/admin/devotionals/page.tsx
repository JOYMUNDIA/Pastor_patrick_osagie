...
export default function AdminDevotionalsPage() {
  ...
  const fetchList = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/devotionals?month=${viewMonth}`);
      const data = await res.json();
      // Fetch full records for the month
      if (data.data && data.data.length > 0) {
        const full = await Promise.all(
          data.data.map((d: { id: string }) => fetch(`/api/devotionals/${d.id}`).then(r => r.json()).then(r => r.data))
        );
